  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1)
	{
		if (timer_flgs.flg_timeout_end)
		{
			//		  GPIO_LED_Toggle();
			TIM1_timeout(500);
//			ax_led(AX_MOTOR1, state);
//			ax_led(AX_MOTOR2, 1 - state);
			ax_set_position_and_speed(5,0,40);
			ax_set_position_and_speed(6,0,4);
			state = 1 - state;

			//ax_goal_position(AX_MOTOR1, position);
			//ax_goal_position(AX_MOTOR2, position);
			...
			servo_position = (servo_position + 0.1) < 1.0 ? servo_position + 0.1 : 0.0;
		}
